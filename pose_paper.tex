...
\title{6DOF Pose Tracking of Rigid Bodies using ARToolkit }
...
For validation of the coupled dynamics of base and manipulator movement during testing, an easy to use system to track the position and orientation, or pose, of the satellite base in 6DOF is required. 
We present an application designed to track the position and orientation of a rigid body with full 6DOF motion range using multiple non co-planar fiducial markers. The framework utilizes ARToolkit, a library for augmented reality applications, and is built on top of ar\_pose, a ROS (Robot Operating System) wrapper for ARToolkit. 
...
Tracking the position and orientation of an object in real time is a problem that has several applications and uses. \cite{lepetit2005monocular} gives a broad overview of monocular, model based methods for 3D tracking of rigid objects, dividing the methods into marker and non-marker based. 
For marker-free tracking, \cite{gordon2006and} uses feature extraction from several images to build an internal 3d model, then tracks pose using this model.
...
ARToolkit is a widely used library that enables marker based pose tracking. Much work has been done in using ARToolkit for pose tracking. \cite{piekarski2002using} uses ARToolkit to track the 3D position in real world coordinates of a user’s hand for mobile applications; \cite{wagner2007artoolkitplus} optimizes ARToolkit to produce ARToolkitPlus, a library tailored toward pose tracking on mobile devices.

ARToolkit uses planar fiducial markers to track the position and orientation of the marker relative to the camera frame, and is easy to use and set up, requiring only a camera and the printed markers. However, there is a limit to the range of rotation where the fiducial is still recognizable - \cite{abawi2004accuracy} indicates that once the camera's relative angle to the marker is past 80 degrees (where 0 degrees is a full, non-angled view of the fiducial), the fiducial becomes undetectable. This becomes an issue when attempting to track an object that may rotate more than the detectable amount. 

Our application is designed to track the pose of a rigid body through a full rotational, 6DOF space (how to word this??). We extend ar\_pose, an existing Robot Operating System (ROS) package, to track an object using multiple attached, non co-planar fiducial markers, thus overcoming the limitation of pattern visibility and extending the trackable range of rotation. 

In section \ref{sec:problem}, we present our problem description. In section \ref{sec:implementation}, we describe package implementation. We conclude in section \ref{sec:future} with a description of future work.

\section{Problem Description}
\label{sec:problem}
(I REALLY don’t know where to put this.. hard to fit in the intro, but clunky in its own section)

The DymaFlight satellite is a microsatellite consisting of a base and an attached manipulator. 
As a microsatellite, DymaFlight is smaller, lighter, faster, and lower cost due to its small size and mass; however, the size introduces significant coupled dynamics between the base and manipulator. 
...
Pose data of the satellite base will be gathered during testing, requiring an efficient and low overhead pose tracking application capable of tracking in 6DOF space. 


\section{Implementation}
\label{sec:implementation}
Our application is implemented using Robot Operating System (ROS). ROS is a library designed for robotic applications, and makes it easy for applications with several independent modules called nodes to communicate with each other. In addition, ROS provides a variety of visualization tools and libraries divided conceptually into packages. 

The ar\_pose package \cite{arpose}  is a ROS wrapper for the ARToolkit library. 
...
\begin{algorithm*}
\caption{Multi Marker Detection - modified}\label{alg:multi}
\begin{algorithmic}[1]
...
We implement an additional node to track a single object using multiple markers in algorithm \ref{alg:multi}. Our implementation modifies the original tracking algorithm to output the final, “master” visualization representing the pose of the actual tracked object instead of the individual markers. 
...
ARToolkit provides the transformation matrix representing the transform of a given marker with respect to the camera frame. To calculate the final transform of the master frame with respect to the camera frame, we //
