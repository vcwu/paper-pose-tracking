...
\begin{figure}[h]
	\includegraphics [width=1\columnwidth] {dymaflight}
		\caption{DymaFlight satellite}
			\label{fig:dymaflight}
\end{figure}
...
During parabolic flight, a series of parabolic arcs are executed to produce short periods of simulated weightlessness lasting around 25 seconds \cite{karmali2008dynamics}.  
...
The {\tt ar\_pose} package is a ROS wrapper for the ARToolkit library. 
It provides two nodes that uses a video feed from a camera to track single and multiple markers.

\begin{figure}[h]
	\includegraphics [width=1\columnwidth] {artoolkit-cs}
		\caption{Transforms from arGetTransMat()}
			\label{fig:ar-cs}
\end{figure}


ARToolkit provides a function {\tt arGetTransMat()}that calculates the transformation matrix representing the transform of the camera frame with respect to a given marker frame(see Figure \ref{fig:ar-cs}). 
Once the transforms of the markers is retrieved, the original node outputs these transforms and displays the visualization for each marker. 

We implement an additional node, {\tt ar\_pose\_bundle}, to track a single object using multiple markers (see Algorithm \ref{alg:multi} and Figure \ref{fig:bundle-overview}). 
Our implementation modifies the original tracking algorithm to output the final, “master” visualization representing the pose of the actual tracked object instead of the individual markers.

\begin{figure}[h]
	\includegraphics [width=1\columnwidth] {pose-bundle-arch}
		\caption{ar\_pose\_bundle overview}
			\label{fig:bundle-overview}
\end{figure}
...
ARToolkit provides the transformation matrix representing the transform of the camera frame with respect to a given marker frame. This transform is inverted to produce the transform of the marker frame with respect to the camera frame. To calculate the final transform of the master frame with respect to the camera frame, we compose the two known transforms as seen in equation \ref{eq:transform}.
\begin{equation}\label{eq:transform}
^{cam}_{master}T = ^{cam}_{marker}T * ^{marker}_{master}T
\end{equation}

\begin{figure}[h]
	\includegraphics [width=1\columnwidth] {modified-cs}
		\caption{Finding master transform}
			\label{fig:modified-cs}
\end{figure}
...
A scaled down rectangular box was used for testing to simulate the base of the microsatellite. 
Attached to each of the four outward sides of the box was a different fiducial; the fiducials' pattern information, size in millimeters, and pose in relation to the base center was encoded in the configuration files. 
Pose output was visualized using rviz, a built in ROS package for visualization. 
Due to different coordinate system conventions between ARToolkit and ROS, an extra transform was introduced before the visualization step to convert the pose from ARToolkit convention (z forward, x right, y down) to ROS convention (x forward, y left, z up). 

Figure \ref{fig:rviz} shows a screenshot of the rviz output when running the application with a live camera feed; each marker's pose in the camera frame is represented by colored cubical markers, while the final master pose representing the center of the box is respresented by a 3D axis. 
...
\begin{figure}[h]
	\includegraphics [width=1\columnwidth] {rviz-output}
		\caption{Visualization of pose output using rviz}
			\label{fig:rviz}
\end{figure}
